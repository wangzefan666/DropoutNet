...
def main():
    ...
    n_scores_user = 2500  # num of samples of every user
    data_batch_size = 100
    ...
    timer.tic()
    eval_warm.init_tf(u_pref_scaled, v_pref_scaled, None, item_content, eval_batch_size)
        ...
        for epoch in range(num_epoch):
            ...
            for b in utils.batch(row_index, user_batch_size):
                ...
                if n_step % eval_every == 0:
                    ...
                    # checkpoint
                    if np.sum(recall_warm) > np.sum(best_warm):
...
def load_data(data_path):
    ...
    u_file = os.path.join(data_path, 'trained/warm/WRMF_warm_rank200_reg1_alpha10_iter10.U.txt')
    v_file = os.path.join(data_path, 'trained/warm/WRMF_warm_rank200_reg1_alpha10_iter10.V.txt')
    item_content_file = os.path.join(data_path, 'item_features_0based.txt')
    train_file = os.path.join(split_folder, 'train.csv')
    test_warm_file = os.path.join(split_folder, 'test.csv')
    test_warm_iid_file = os.path.join(split_folder, 'test_item_ids.csv')
